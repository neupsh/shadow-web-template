...
(rf/reg-sub
  :example-1-value1
  (fn [_ _]
    (rf/subscribe [:example-1-value1-pre]))
  (fn [value _]
...
(rf/reg-sub
  :example-1-value2
  (fn [_ _]
    (rf/subscribe [:firestore/on-snapshot {:path-document [:sample-collection :sample-document]}]))
  (fn [value _]
    (with-out-str (pprint value))))
